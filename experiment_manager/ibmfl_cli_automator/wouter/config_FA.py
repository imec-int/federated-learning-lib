...
#TODO added for FA script
def get_FA_script_from_config(config):
    """
    Load Federated Analytics class information from params provided in config file.
    ...
    if config:
        try:
            FA_script_config['cls_ref'] = get_class_by_name(
                config['path'], config['name'])
            if 'info' in config:
                FA_script_config['info'] = config['info']
        except Exception as ex:
            logger.exception(ex)
